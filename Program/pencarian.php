<?php include "koneksi.php"; ?>
<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Pendataan Penduduk</title>

    <!-- Custom fonts for this template-->
    <link href="vendor/fontawesome-free/css/all.min.css" rel="stylesheet" type="text/css">
    <link
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
        rel="stylesheet">

    <!-- Custom logo -->
    <link rel="icon" href="img/lambang3.png">

    <!-- Custom styles for this template-->
    <link href="css/sb-admin-2.min.css" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="css/style.css">

</head>

<body id="page-top">

    <!-- Page Wrapper -->
    <div id="wrapper">

        <!-- Sidebar -->
        <ul class="navbar-nav samping sidebar sidebar-dark accordion" id="accordionSidebar">

            <!-- Sidebar - Brand -->
            <a class="sidebar-brand d-flex align-items-center justify-content-center" href="index.php">
                <div class="sidebar-brand-icon logo">
                </div>
                <div class="sidebar-brand-text mx-3 text">Pendataan Penduduk</div>
            </a>

            <!-- Divider -->
            <hr class="sidebar-divider my-0">

            <!-- Nav Item - Dashboard -->
            <li class="nav-item">
                <a class="nav-link" href="index.php">
                    <i class="fas fa-fw fa-home"></i>
                    <span>Dashboard</span></a>
            </li>

            <!-- Divider -->
            <hr class="sidebar-divider my-0">

            <!-- Nav Item - Dashboard -->
            <li class="nav-item">
                <a class="nav-link" href="datas_user.php">
                    <i class="fas fa-fw fa-clone"></i>
                    <span>Semua Data</span></a>
            </li>

            <!-- Nav Item - Dashboard -->
            <li class="nav-item">
                <a class="nav-link" href="all_data_user.php">
                    <i class="fas fa-fw fa-file"></i>
                    <span>Data Penduduk</span></a>
            </li>

            <!-- Nav Item - Dashboard -->
            <li class="nav-item">
                <a class="nav-link" href="all_data_pendatang_user.php">
                    <i class="fas fa-fw fa-file"></i>
                    <span>Data Pendatang</span></a>
            </li>

            <!-- Divider -->
            <hr class="sidebar-divider my-0">

            <!-- Nav Item - Dashboard -->
            <li class="nav-item">
                <a class="nav-link" href="all_data_kep_user.php">
                    <i class="fas fa-fw fa-folder-open"></i>
                    <span>Data Kepala Keluarga</span></a>
            </li>

            <!-- Nav Item - Dashboard -->
            <li class="nav-item">
                <a class="nav-link" href="all_data_pin_user.php">
                    <i class="fas fa-fw fa-folder-open"></i>
                    <span>Data Penduduk Pindah</span></a>
            </li>

            <!-- Divider -->
            <hr class="sidebar-divider d-none d-md-block">

            <!-- Sidebar Toggler (Sidebar) -->
            <div class="text-center d-none d-md-inline">
                <button class="rounded-circle border-0" id="sidebarToggle"></button>
            </div>

        </ul>
        <!-- End of Sidebar -->

        <!-- Content Wrapper -->
        <div id="content-wrapper" class="d-flex flex-column">

            <!-- Main Content -->
            <div id="content">

                <!-- Topbar -->
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">

                    <!-- Sidebar Toggle (Topbar) -->
                    <button id="sidebarToggleTop" class="btn btn-link d-md-none rounded-circle mr-3">
                        <i class="fa fa-bars"></i>
                    </button>

                    <!-- Topbar Search -->
                    <form
                        class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" action="pencarian.php" method="POST" autocomplete="off">
                        <div class="input-group">
                            <input type="text" class="form-control bg-light border-0 small" placeholder="Cari berdasarkan NIK . . . ."
                                aria-label="Search" aria-describedby="basic-addon2" name="inputnik" required="">
                            <div class="input-group-append">
                                <button class="btn btn-primary" type="submit" name="submit">
                                    <i class="fas fa-search fa-sm"></i>
                                </button>
                            </div>
                        </div>
                    </form>

                    <!-- Topbar Navbar -->
                    <ul class="navbar-nav ml-auto">

                        <div class="topbar-divider d-none d-sm-block"></div>

                        <!-- Nav Item - User Information -->
                        <li class="nav-item dropdown no-arrow mx-1">
                            <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button"
                                data-toggle="modal" aria-haspopup="true" aria-expanded="false" data-target="#logoutModal">
                                <i class="fas fa-power-off fa-fw"></i>
                            </a>
                        </li>

                    </ul>

                </nav>
                <!-- End of Topbar -->

                <!-- Begin Page Content -->
                <div class="container-fluid">

                    <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800"></h1>
                    </div>

                    <!-- Bagian Searching -->
                    <div class ="container">
                            <div class="row">

                            <?php
                                                
                                $nilai_nik  = $_POST['inputnik'];

                                $result1= mysqli_query($conn, "SELECT * FROM data_penduduk WHERE nik = '$nilai_nik'");
                                $row1=mysqli_fetch_array($result1);

                                $result2= mysqli_query($conn, "SELECT * FROM data_pendatang WHERE nik = '$nilai_nik'");
                                $row2=mysqli_fetch_array($result2);

                                $result3= mysqli_query($conn, "SELECT * FROM data_penduduk_pindah WHERE nik = '$nilai_nik'");
                                $row3=mysqli_fetch_array($result3);
                                                        
                                if ($nilai_nik == $row1['nik'])
                                {
                                ?>
                                <div class="col-lg-6">
                                    <!-- Overflow Hidden -->
                                    <div class="card mb-4">
                                        <div class="card-header py-3 bg-success">
                                            <h6 class="m-0 font-weight-bold text-light">Anda Telah Terdaftar (Penduduk Tetap)</h6>
                                        </div>
                                        <div class="card-body">
                                            <strong><i class='fas fa-book mr-1'></i> NIK</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['nik'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-user mr-1'></i> Nama</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['nama'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-pray mr-1'></i> Agama</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['agama'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-venus-mars mr-1'></i> Jenis Kelamin</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['gender'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-address-book mr-1'></i> Tanggal Lahir</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['ttl'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-hotel mr-1'></i> Dusun</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['dusun'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-map-marker-alt mr-1'></i> Alamat</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['alamat'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-graduation-cap mr-1'></i> Pendidikan</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['pendidikan'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-user-tie mr-1'></i> Pekerjaan</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['pekerjaan'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-heart mr-1'></i> Golongan Darah</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['gol_darah'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-id-card mr-1'></i> No Telp</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['no_telp'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-smile mr-1'></i> Status</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['status'] ?>
                                            </p>

                                            <hr>

                                            <strong><i class='fas fa-bookmark mr-1'></i> Kategori</strong>
                                            <p class='text-muted'>
                                                                          
                                            </p>
                                            <p class='text-muted'>
                                            <?php  echo $row1['kategori'] ?>
                                            </p>

                                            <hr>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-lg-6">

                                <!-- Roitation Utilities -->
                                <div class="card">
                                    <div class="card-header py-3">
                                        <h6 class="m-0 font-weight-bold text-primary">Surat Pengajuan</h6>
                                    </div>
                                    <!-- Target blank untuk membuka tab baru-->
                                    <div class="card-body text-left">
                                        <a href="<?php echo "cetak_surat_ket_domisili.php?nik=".$row1['nik']; ?>" class="btn btn-primary btn-icon-split" target="_blank">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-print"></i>
                                            </span>
                                            <span class="text">Surat Keterangan Domisili</span>
                                        </a>
                                        <hr>
                                        <a href="<?php echo "cetak_surat_ket_kurang_mampu.php?nik=".$row1['nik']; ?>" class="btn btn-primary btn-icon-split" target="_blank">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-print"></i>
                                            </span>
                                            <span class="text">Surat Keterangan Tidak Mampu</span>
                                        </a>
                                        <hr>
                                        <a href="<?php echo "cetak_surat_ket_usaha.php?nik=".$row1['nik']; ?>" class="btn btn-primary btn-icon-split" target="_blank">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-print"></i>
                                            </span>
                                            <span class="text">Surat Keterangan Usaha</span>
                                        </a>
                                        <hr>
                                        <a href="<?php echo "cetak_surat_ket_pindah_domisili.php?nik=".$row1['nik']; ?>" class="btn btn-primary btn-icon-split" target="_blank">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-print"></i>
                                            </span>
                                            <span class="text">Surat Keterangan Pindah Domisili</span>
                                        </a>
                                    </div>
                                </div>

                                </div>
                            <?php
                            }
                            else if ($nilai_nik == $row2['nik'])
                            {
                            ?>
                            <div class="col-lg-6">
                                <!-- Overflow Hidden -->
                                <div class="card mb-4">
                                    <div class="card-header py-3 bg-info">
                                        <h6 class="m-0 font-weight-bold text-light">Anda Telah Terdaftar (Penduduk Pendatang)</h6>
                                    </div>
                                    <div class="card-body">
                                        <strong><i class='fas fa-book mr-1'></i> NIK</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['nik'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-user mr-1'></i> Nama</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['nama'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-pray mr-1'></i> Agama</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['agama'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-venus-mars mr-1'></i> Jenis Kelamin</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['gender'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-address-book mr-1'></i> Tanggal Lahir</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['ttl'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-hotel mr-1'></i> Dusun</strong>
                                        <p class='text-muted'>
                                                                          
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['dusun'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-map-marker-alt mr-1'></i> Alamat</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['alamat'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-graduation-cap mr-1'></i> Pendidikan</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['pendidikan'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-user-tie mr-1'></i> Pekerjaan</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['pekerjaan'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-heart mr-1'></i> Golongan Darah</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['gol_darah'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-id-card mr-1'></i> No Telp</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['no_telp'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-smile mr-1'></i> Status</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['status'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-bookmark mr-1'></i> Kategori</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row2['kategori'] ?>
                                        </p>

                                        <hr>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-6">

                                <!-- Roitation Utilities -->
                                <div class="card">
                                    <div class="card-header py-3">
                                        <h6 class="m-0 font-weight-bold text-primary">Surat Pengajuan</h6>
                                    </div>
                                    <div class="card-body text-left">
                                        <a href="<?php echo "cetak_surat_ket_usaha2.php?nik=".$row2['nik']; ?>" class="btn btn-primary btn-icon-split" target="_blank">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-print"></i>
                                            </span>
                                            <span class="text">Surat Keterangan Usaha</span>
                                        </a>
                                    </div>
                                </div>

                            </div>
                            <?php
                            }
                            else if ($nilai_nik == $row3['nik'])
                            {
                            ?>
                            <div class="col-lg-6">
                                <!-- Overflow Hidden -->
                                <div class="card mb-4">
                                    <div class="card-header py-3 bg-warning">
                                        <h6 class="m-0 font-weight-bold text-light">Anda Telah Terdaftar (Penduduk Pindah)</h6>
                                    </div>
                                    <div class="card-body">
                                        <strong><i class='fas fa-book mr-1'></i> NIK</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['nik'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-user mr-1'></i> Nama</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['nama'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-pray mr-1'></i> Agama</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['agama'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-venus-mars mr-1'></i> Jenis Kelamin</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['gender'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-address-book mr-1'></i> Tanggal Lahir</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['ttl'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-hotel mr-1'></i> Dari Dusun</strong>
                                        <p class='text-muted'>
                                                                          
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['dusun'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-map-marker-alt mr-1'></i> Alamat</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['alamat'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-graduation-cap mr-1'></i> Pendidikan</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['pendidikan'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-user-tie mr-1'></i> Pekerjaan</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['pekerjaan'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-heart mr-1'></i> Golongan Darah</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['gol_darah'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-id-card mr-1'></i> No Telp</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['no_telp'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-smile mr-1'></i> Status</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['status'] ?>
                                        </p>

                                        <hr>

                                        <strong><i class='fas fa-bookmark mr-1'></i> Kategori</strong>
                                        <p class='text-muted'>
                                                                      
                                        </p>
                                        <p class='text-muted'>
                                        <?php  echo $row3['kategori'] ?>
                                        </p>

                                        <hr>
                                    </div>
                                </div>
                            </div>
                            <?php
                            }
                            else
                            {
                            ?>
                                <div class="col-lg-6">
                                <!-- Overflow Hidden -->
                                <div class="card mb-4">
                                    <div class="card-header py-3 bg-danger">
                                        <h6 class="m-0 font-weight-bold text-light">Anda Tidak Terdaftar</h6>
                                    </div>
                                    <div class="card-body">
                                </div>
                                </div>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                                <br>
                            <?php               
                            }
                            ?>
                            </div>
                    </div>
                    <br>
                    <br>
                    <br>
                    <br>
                <!-- /.container-fluid -->

            </div>
            <!-- End of Main Content -->

            <!-- Footer -->
            <footer class="sticky-footer">
                <div class="container my-auto">
                    <div class="copyright text-center my-auto">
                        <span>Copyright &copy; Kantor Desa Tanampedagi</span>
                    </div>
                </div>
            </footer>
            <!-- End of Footer -->

        </div>
        <!-- End of Content Wrapper -->

    </div>
    <!-- End of Page Wrapper -->

    <!-- Scroll to Top Button-->
    <a class="scroll-to-top rounded" href="#page-top">
        <i class="fas fa-angle-up"></i>
    </a>

    <!-- Logout Modal-->
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title kelas_login" id="exampleModalLabel">
                        Login
                    </h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form autocomplete="off" action="action_login.php" method="POST">
                        <div class="form-group">
                            <input type="text" class="form-control" name="username" placeholder="Username">
                        </div>
                        <div class="form-group">
                            <input type="password" class="form-control" name="password" placeholder="Password">
                        </div>
                        <div class="modal-footer">
                            <button class="btn btn-primary" name="submit">Login
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>

    <!-- Bootstrap core JavaScript-->
    <script src="vendor/jquery/jquery.min.js"></script>
    <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="vendor/jquery-easing/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="js/sb-admin-2.min.js"></script>

    <!-- Page level plugins -->
    <script src="vendor/chart.js/Chart.min.js"></script>

    <!-- Page level custom scripts -->
    <script src="js/demo/chart-area-demo.js"></script>
    <script src="js/demo/chart-pie-demo.js"></script>

</body>

</html>